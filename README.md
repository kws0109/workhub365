...
**개발 기간과 내 역할** — 2026-08-07~09, 커밋 76개, 8개 모듈 14화면. 코드 생성은 에이전트가 했고, 제품·스펙 결정, 다관점 병렬 리뷰 하네스 설계, 실브라우저 E2E 검증, 커밋 단위 통제는 제 몫이었습니다. 에이전트 산출물을 그대로 받지 않았다는 근거:
...
| 비즈니스 로직은 순수 함수로 분리 | 낭비 금액 계산·상태머신·52시간 집계를 DB/Graph 없이 단위 테스트 (Vitest 249개) |
